...
2. **权限管理**：自动处理不同 Android 版本的照片访问权限
3. **相册分类**：支持按相册浏览图片
4. **多选支持**：可配置单选或多选模式
5. **搜索功能**：支持在媒体文件中搜索
...
- Android 12L 及以下使用 `READ_EXTERNAL_STORAGE`。
- Android 13 使用 `READ_MEDIA_IMAGES`。
- Android 14 及以上同时处理完整照片访问和 `READ_MEDIA_VISUAL_USER_SELECTED` 部分照片访问。

当用户只授予部分照片访问时，MediaStore 只向选择器返回已授权的图片；该状态仍被视为可用权限，而不是权限拒绝。
